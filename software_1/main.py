...
# get random color
color = {k: (random.randint(0, 255), random.randint(0, 255), random.randint(0, 255)) for k in range(total_template_no)}
...
def get_templates_weights(frame_to_get_templ, templ_coords):
    cnt_len_list = []
    ...
    for t_c in templ_coords:
        ...
        cnt_len_list.append(len(cnts) if len(cnts) > 0 else 1)
    ...
    while True:
        ...
        if startPoint is True and endPoint is True:
            ...
            if not any(template_dict) or isTemplateMatchCoordLen(template_match_coord_dict,
                ...
                weights = get_templates_weights(frame_gray, templates_coord)
