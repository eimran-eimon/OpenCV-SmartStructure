...
parser.add_argument("-r", "--record_video", dest="record_video", help="Record Video, default 0")
parser.add_argument("-u", "--unit", dest="unit", help="Unit, default 1ft")
...
if input_file is not None:
	if input_file == 'camera':
		...
		if os.path.isfile(input_file) and input_file.lower().endswith((".mp4", ".mkv")):
			...
			exit(1)
	record_video = vars(args)['record_video'] if vars(args)['record_video'] else 0
	unit = vars(args)['unit'] if vars(args)['unit'] else 1
	...
	record_video = config['record'] if config['record'] else 0
	unit = config['unit'] if config['unit'] else 1
...
# csv field names
data_fields = ["Date-Time", "Penetration (in ft)"]
csv_directory = "./stored_csv_files"
recorded_video_dir = f"./stored_video_file"

if not os.path.exists(csv_directory):
	os.makedirs(csv_directory)
if not os.path.exists(recorded_video_dir) and config['record'] == 1:
	os.makedirs(recorded_video_dir)
# name of the csv file
data_filename = f"./{csv_directory}/data_{datetime.now().strftime('%Y_%m_%d_%H_%M_%S')}.csv"
recorded_video_filename = f"./{recorded_video_dir}/{datetime.now().strftime('%Y_%m_%d_%H_%M_%S')}.mkv"
...
with open(data_filename, 'w', newline='', encoding='utf-8') as csv_file:
	...
	while True:
		...
		# if the video writer has not been initialized
		if writer is None and int(record_video) == 1:
			# initialize our video writer
			fourcc = cv2.VideoWriter_fourcc(*"MJPG")
			writer = cv2.VideoWriter(recorded_video_filename, fourcc, 30,
			                         (frame_copy.shape[1], frame_copy.shape[0]), True)
		
		# if the video writer is not None, write the frame to the output
		# video file
		if writer is not None and int(record_video) == 1:
		...
		if startPoint is True and endPoint is True:
			if red_line_dist is None:
				red_line_dist = np.abs(rect[1] - rect[3]) / int(unit)
			...
			cv2.putText(frame, "Penetration: {:.2f}ft".format(sinked), (50, 30), cv2.FONT_HERSHEY_SIMPLEX, 1,
			            (0, 0, 255), 2)
		...
		if red_line_dist is None:
			instruction_text = f"Please select {unit}ft area on the Piller"
