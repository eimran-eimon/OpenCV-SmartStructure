...
def run(im):
	...
	def callback(event, x, y, flags, param):
		if event == cv2.EVENT_LBUTTONDOWN:
			run.mouse_down = True
			pts_1.append((x, y))
		elif event == cv2.EVENT_LBUTTONUP and run.mouse_down is True:
			run.mouse_down = False
			pts_2.append((x, y))
			print("Object selected at [{}, {}]".format(pts_1[-1], pts_2[-1]))
		elif event == cv2.EVENT_MOUSEMOVE and run.mouse_down is True:
	...
	while True:
		...
		if key == ord('p'):
			...
			if run.mouse_down is False and pts_1:
